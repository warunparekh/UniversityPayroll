...
function TaxSlabApp() {
    ...
    function handleSubmit(ev) {
        ...
        formData.slabs = formData.slabs.filter(function(s) { return s.rate > 0; });
        formData.id = editingId;
        fetch(url, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(formData) }).then(function() { window.location.reload(); });
    }
    ...
    function updateSlab(i, field, val) {
        ...
        for (var j = 0; j < formData.slabs.length; j++) {
          var newSlab = {};
          newSlab.from = formData.slabs[j].from;
          newSlab.to = formData.slabs[j].to;
          newSlab.rate = formData.slabs[j].rate;
          newSlabs[j] = newSlab;
        }
    ...
    function showEditForm(id) {
        fetch("/TaxSlab/GetById?id=" + id).then(function(res) { return res.json(); }).then(function(data) {
